<script>
  ...
  import { user, status as userStatus } from "../../../stores/user";
  ...
  $: if ($userStatus === "DELETED") $redirect("../users");
...
{:else}
  ...
  <div class="flex items-baseline justify-between">
    <div
      ...
      <label
        ...
        class="block px-4 py-2 leading-none border-r border-teal-500
          cursor-pointer {tab === 'personal_data' ? 'bg-teal-500 text-white' : 'text-teal-500'}">
        Persönliche Daten
      </label>
      ...
      {#if $user.role === 'STUDENT'}
        ...
        <label
          for="classes"
          class="block px-4 py-2 leading-none border-r border-teal-500
            cursor-pointer {tab === 'classes' ? 'bg-teal-500 text-white' : 'text-teal-500'}">
          Klassen
        </label>
    ...
    <div>
      {#if tab === 'personal_data'}
        <button
          on:click={() => user.del(id)}
          class="text-red-500 hover:text-red-600">Benutzer löschen</button>
      {/if}

      {#if $user.role === 'STUDENT' && tab === 'invoices'}
        <button
          on:click={() => (showNewInvoiceModal = !showNewInvoiceModal)}
          class="p-2 text-teal-700 bg-teal-100 rounded-full focus:outline-none hover:bg-teal-200">
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
  ...
  {#if tab === 'personal_data'}
    <form on:submit|preventDefault={user.update}>
      ...
      <div class="mb-5">
        ...
        <input type="text" bind:value={$user.phone_number} id="phone_number" />
      </div>
      ...
      <button
        ...
        {$userStatus === 'UPDATING' ? 'Bitte warten...' : 'Speichern'}
      </button>
    </form>
  {:else if tab === 'classes'}
    <div>
      <h2>Eingeschriebene Klassen</h2>
      ...
      <div class="overflow-hidden border rounded shadow-md">
        <header
          ...
          <div class="w-6/12">Fach</div>
          <div class="w-5/12 text-center">Lehrer</div>
          <div class="w-1/12 text-center">Schüler</div>
        </header>
        <div class="bg-white">
          {#each $classes.filter((c) =>
            c.enrollments.some((e) => e.student.id === id)
          ) as clss, index (clss.id)}
            <a
              href={$url('../classes/' + clss.id)}
              class="flex flex-wrap px-6 py-3 text-gray-800 border-b hover:bg-gray-100">
              <div class="w-6/12">
                <div>{clss.subject}</div>
                <div class="text-sm text-gray-700">
                  {getTimeString(clss.times)}
                </div>
              </div>

              <div
                class="flex items-center justify-center w-5/12 text-gray-700">
                <div>
                  {clss.teacher.first_name + ' ' + clss.teacher.last_name}
                </div>
              </div>
    ...
    {#if $user.invoices.length > 0}
      <div class="overflow-hidden border rounded shadow-md">
        <header
          ...
          <div class="w-3/12 text-center">Betrag</div>
          <div class="w-3/12 text-center">Datum</div>
  ...
  {:else if tab === 'comments'}
    <header class="flex justify-end mb-8">
      <button
        on:click={() => (showNewCommentModal = !showNewCommentModal)}
    ...
    {#if showNewCommentModal}
      <form on:submit|preventDefault={createComment} class="mb-8">
        ...
        <button
          class="btn btn--primary"
          disabled={$userStatus === 'CREATING_COMMENT'}>
          {$userStatus === 'CREATING_COMMENT' ? 'Einen Moment...' : 'Speichern'}
        </button>
      </form>
    {/if}
  ...
  <CreateInvoiceModal student={$user} show={showNewInvoiceModal} />
{/if}
